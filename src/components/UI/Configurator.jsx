...
import { pen_Parts } from "../../constants";
...
const Configurator = () => {
  ...
  return (
    ...
    >
      ...
      >
        {pen_Parts.map(({ partName, data, valueToChange }, index) => (
          <div className="flex flex-col" key={index}>
            <h3 className={``}>{partName}</h3>
              ...
              {data.map(({ name, color }, index) => (
                ...
                >
                  <div
                    ...
                    onClick={() => (store.modelConfigurator[valueToChange] = { name, color })}
                  />
                </div>
              ))}
            </div>
